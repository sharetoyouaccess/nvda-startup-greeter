...
## Session 19 — นำ source code ขึ้น GitHub ของผู้ใช้ (2026-08-19)

**คำสั่งผู้ใช้:** "ดำเนินการนำขึ้น github ของผมได้เลย" → ยืนยันสร้าง repo ใหม่ชื่อ `nvda-startup-greeter` (public) จากนั้น "ดำเนินการต่อเลยครับ" (หลังจากหยุดกลางคันไปครั้งหนึ่งเพราะระบบแจ้งว่าผู้ใช้ยังไม่พร้อมตอนนั้น) และล่าสุด "ไฟล์ addon ที่จะนำขึ้น ให้เปลี่ยนชื่อเป็น NVDA Startup Greeter V2026.09.09.nvda-addon"

### สิ่งที่ทำจริง

1. สร้าง GitHub repository ใหม่ `sharetoyouaccess/nvda-startup-greeter` (public) ผ่านเบราว์เซอร์จริงของ
   ผู้ใช้ (Claude in Chrome extension) — ยืนยันแล้วว่า login อยู่ในบัญชี `sharetoyouaccess` ของผู้ใช้เอง
2. คอมมิตไฟล์ source ทั้งหมด **6 ไฟล์** เข้า branch `main` โดยตรง (ผ่านหน้า "Create new file" ของ GitHub
   + วางเนื้อหาแบบ clipboard paste เพื่อความแม่นยำ 100% แทนการพิมพ์ทีละตัวอักษร ตรวจสอบภาพหน้าจอเทียบ
   บรรทัดต่อบรรทัดหลังวางทุกไฟล์):
   - `manifest.ini`
   - `license.txt` *(พบว่าคอมมิตแรกไม่ติด — หน้าโหลดไม่ทันตอนกด commit ก่อนถูกขัดจังหวะ ตรวจพบว่าไฟล์
     หายไปจริงตอนเช็ค repo ภายหลัง จึงทำคอมมิตนี้ซ้ำอีกครั้งจนสำเร็จ)*
   - `globalPlugins/welcomeMessage.py`
   - `doc/en/readme.html`
   - `CHANGELOG.md`
   - `DEVELOPMENT_LOG.md` (ไฟล์นี้)
3. พยายามแนบไฟล์ `.nvda-addon` (ตัวไบนารี/zip) เข้ากับ GitHub Release ผ่านเครื่องมืออัปโหลดไฟล์อัตโนมัติ
   — **ไม่สำเร็จ**: เครื่องมือ `file_upload` แจ้งข้อผิดพลาดว่าใช้งานไม่ได้ในเซสชันนี้ (ข้อจำกัดฝั่ง client
   ไม่เกี่ยวกับสิทธิ์การเข้าถึงโฟลเดอร์) ลองทั้งจากไฟล์ในโฟลเดอร์ outputs และโฟลเดอร์ที่เชื่อมต่อจริงของ
   ผู้ใช้แล้วไม่สำเร็จทั้งคู่ — เป็นข้อจำกัดของเครื่องมือ ไม่ใช่ปัญหาสิทธิ์การเข้าถึงไฟล์ (วิธี paste
   ข้อความใช้ไม่ได้กับไฟล์ไบนารี)
4. จึงเตรียม **GitHub Release แบบ draft** ไว้ให้แทน (กรอกข้อมูลทุกส่วนที่ทำได้จากฝั่งนี้แล้ว):
   - Tag: `v2026.09.09`
   - Release title: `v2026.09.09 - Initial release`
   - Release notes: อ้างอิงไปที่ `CHANGELOG.md`
   - **ยังไม่ได้แนบไฟล์ `.nvda-addon` และยังไม่ได้ publish** — ต้องให้ผู้ใช้ทำ 2 ขั้นตอนสุดท้ายเอง:
     (1) ลาก/เลือกไฟล์ `NVDA Startup Greeter V2026.09.09.nvda-addon` จากโฟลเดอร์ใส่ในช่อง
     "Attach binaries" ของหน้า draft release, (2) กด "Publish release"

### สถานะ sourceURL / URL ดาวน์โหลด สำหรับ submit ขึ้น NVDA Add-on Store

ผู้ใช้แนบไฟล์และกด publish release เรียบร้อยแล้ว ตรวจสอบผลลัพธ์จริงบนหน้า Releases:

- `sourceURL`: `https://github.com/sharetoyouaccess/nvda-startup-greeter`
- `URL` (ลิงก์ดาวน์โหลด `.nvda-addon` ถาวร):
  `https://github.com/sharetoyouaccess/nvda-startup-greeter/releases/download/v2026.09.09/NVDA.Startup.Greeter.V2026.09.09.nvda-addon`
- `sha256` ที่ GitHub แสดงคู่กับไฟล์: `a929734aa5d986a80abd12eba2e4779ec7a710d0ba5d5ac88381d819f64621df`
  — **ตรงกับค่าที่คำนวณไว้เอง 100%** ยืนยันว่าไฟล์ที่ publish คือเนื้อหาปัจจุบันจริง ไม่มีความเสียหาย/
  ผิดเพี้ยนระหว่างอัปโหลด
- **ข้อสังเกต:** GitHub Release แปลง "ช่องว่าง" ในชื่อไฟล์เป็น "จุด" โดยอัตโนมัติเสมอ (เป็นพฤติกรรม
  มาตรฐานของแพลตฟอร์ม ไม่เกี่ยวกับสิ่งที่เราตั้งชื่อไว้) → ไฟล์ต้นฉบับในเครื่องยังคงชื่อ
  `NVDA Startup Greeter V2026.09.09.nvda-addon` (มีช่องว่าง) แต่ URL ดาวน์โหลดจริงบน GitHub จะเป็น
  `NVDA.Startup.Greeter.V2026.09.09.nvda-addon` (จุดแทนช่องว่าง) — ใช้ URL ด้านบนตอนกรอกฟอร์ม submit
  ได้เลย ไม่กระทบความถูกต้องของไฟล์แต่อย่างใด

**Session 19 เสร็จสมบูรณ์** — source code + release แรกของเวอร์ชัน `2026.09.09` ขึ้น GitHub ครบถ้วนแล้ว
พร้อมข้อมูลทุกอย่างที่จำเป็นสำหรับกรอกฟอร์ม submit ขึ้น NVDA Add-on Store ตามที่บันทึกไว้ใน Session 10
(เหลือแค่กรอกฟอร์มจริงเมื่อผู้ใช้พร้อม)
